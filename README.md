...
Файлы, которые кандидат присылает в Telegram (резюме, тестовое, доп. документы), автоматически прикрепляются в поле "Резюме" карточки через Notion File Upload API (`notion_service.sync_uploaded_files`) — в дополнение к пересылке этих же файлов напрямую в чат рекрутера. Ограничение: обычный (не self-hosted) Telegram Bot API не отдаёт файлы тяжелее 20 МБ — такие файлы останутся только в чате рекрутера, в тело карточки при этом добавляется пояснение.
